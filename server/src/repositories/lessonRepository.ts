 ...
 * Удаляет урок.
 *
 * План, привязки материалов, реплики, задания и попытки уходят каскадом
 * (внешние ключи в `001_init.sql`). Словарь и журнал ошибок переживают удаление:
 * у них `ON DELETE SET NULL`, потому что выученное слово остаётся выученным,
 * даже если занятие удалили.
 *
 * Следствие для отбора материала: фрагменты, отработанные в этом уроке, снова
 * станут непройденными — признак «пройдено» выводится из завершённых шагов.
 */
export function deleteLessonById(id: Id): boolean {
  return getDb().prepare('DELETE FROM lessons WHERE id = ?').run(id).changes > 0;
}
